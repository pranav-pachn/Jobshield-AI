...
class NVIDIAProvider(LLMProvider):
    ...
    def _map_exception(self, e: Exception) -> Exception:
        ...
        if "429" in err_str or "rate limit" in err_str:
            return ProviderRateLimitError(f"NVIDIA Rate Limit: {str(e)}")
        if "timeout" in err_str or "deadline" in err_str:
            return ProviderTimeoutError(f"NVIDIA Timeout: {str(e)}")
        if "401" in err_str or "403" in err_str or "api key" in err_str:
            return ProviderAuthenticationError(f"NVIDIA Auth Error: {str(e)}")
        return ProviderAPIError(f"NVIDIA API Error: {str(e)}")

    async def generate(self, request: LLMRequest) -> LLMResponse:
        if not self.client:
            raise ProviderAuthenticationError("NVIDIA API key not configured")
        ...
        try:
            ...
            if request.response_model and text_content:
                ...
                try:
                    ...
                    raise ProviderOutputParsingError(f"Failed to parse NVIDIA output: {str(e)}")
