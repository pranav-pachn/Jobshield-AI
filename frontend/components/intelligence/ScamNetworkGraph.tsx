...
function ScamNetworkGraphComponent({ jobAnalysisId, staticData }: ScamNetworkGraphProps) {
  ...
  useEffect(() => {
    if (staticData) {
      ...
      const convertedNodes = staticData.nodes.map((n: any) => ({
        id: n.id,
        type: 'customNode',
        position: { x: Math.random() * 400, y: Math.random() * 400 },
        data: n
      }));
      const convertedEdges = staticData.links.map((l: any, i: number) => ({
        id: `e-${i}`,
        source: l.source,
        target: l.target,
        animated: true,
      ...
      setGraphData({
        ...
        metadata: { totalNetworks: 1, totalLinkedAnalyses: 1, correlationTypes: [] },
        summary: {
          nodeCount: convertedNodes.length,
          edgeCount: convertedEdges.length,
          uniqueEntities: convertedNodes.length,
          correlationSources: [],
          highestConfidence: 0,
          averageConfidence: 0
        }
...
export function ScamNetworkGraph({ jobAnalysisId, staticData }: ScamNetworkGraphProps) {
  return (
    <Card className="glass-card border-border shadow-lg h-full">
      <CardHeader>
      ...
      <CardContent className="p-0 h-[calc(100%-100px)] min-h-[500px]">
        <ReactFlowProvider>
          <ScamNetworkGraphComponent jobAnalysisId={jobAnalysisId} staticData={staticData} />
        </ReactFlowProvider>
      </CardContent>
    </Card>
  );
